...
export default function Home() {
  ...
  // --- START GAME WITH COLLISION DETECTION ---
  const handleStartGame = async () => {
    ...
    while (!validStart) {
      ...
      for (const tile of drawnTiles) {
        ...
        if (neighbors.some(n => drawnTiles.includes(n))) {
      ...
      if (!collisionDetected) {
        ...
        finalUpdates = drawResults.map((p, index) => ({
          id: p.id,
        ...
        console.log("Collision detected in initial draw! Retrying draw phase...");
      }
    }
        ...
        <div className="max-w-7xl mx-auto h-full grid grid-cols-1 lg:grid-cols-12 gap-0 lg:gap-6 lg:p-6">
              ...
              <div className="max-w-md w-full bg-slate-900 p-8 rounded-3xl border border-slate-800 shadow-2xl">
                {view === 'home' && (
                  <div className="flex flex-col gap-4">
                    <button onClick={() => setView('create')} className="bg-amber-500 text-black font-black py-4 rounded-xl hover:bg-amber-400 transition-all">ESTABLISH SYNDICATE</button>
                    <button onClick={() => setView('join')} className="border-2 border-amber-500 text-amber-500 font-black py-4 rounded-xl hover:bg-slate-800 transition-all">INFILTRATE</button>
                  </div>
                )}
                {view === 'create' && (
                ...
                {view === 'join' && (
                  <form onSubmit={handleJoinLobby} className="flex flex-col gap-4">
                    ...
                    <input type="text" maxLength={6} required value={joinCodeInput} onChange={e => setJoinCodeInput(e.target.value.toUpperCase())} className="bg-slate-800 p-4 rounded-xl text-center font-mono tracking-[0.3em]" placeholder="XXXXXX"/>
          ...
          {lobbyInfo && lobbyInfo.status === 'waiting' && (
            <div className="lg:col-span-12 flex flex-col items-center justify-center h-full p-6 text-center">
               ...
               {isHost && players.length >= 2 && <button onClick={handleStartGame} disabled={isStarting} className="bg-emerald-500 px-12 py-4 rounded-2xl font-black animate-pulse shadow-xl shadow-emerald-500/20">{isStarting ? 'INITIATING...' : 'INITIATE OPERATION'}</button>}
            </div>
          )}
            ...
            <>
                ...
                <div className="min-w-[580px] lg:min-w-0">
                  <div className="grid grid-cols-12 gap-1 sm:gap-2">
                    {BOARD_ROWS.map(r => BOARD_COLS.map(c => {
                      ...
                      return (
                        <div key={id} className={`aspect-square flex flex-col items-center justify-center rounded-lg text-[10px] font-bold border-2 transition-all 
              ...
              <div className={`lg:col-span-4 flex flex-col gap-6 p-4 lg:p-0 overflow-y-auto ${mobileTab !== 'market' ? 'hidden lg:flex' : 'flex'}`}>
                <div className="bg-slate-800/50 border border-slate-700 p-4 rounded-2xl shadow-xl">
                  <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">Opening Ceremony Result</h4>
                  <div className="flex flex-wrap gap-4">
                    {players.sort((a,b) => (a.play_order || 0) - (b.play_order || 0)).map(p => (
                      <div key={p.id} className="flex flex-col items-center">
                        <div className="bg-amber-500 text-black font-mono text-xs px-2 py-1 rounded font-bold shadow-lg">
                          {p.starting_tile}
                        </div>
              ...
              <div className={`lg:col-span-4 flex flex-col bg-slate-900 lg:rounded-3xl lg:border border-slate-800 h-full shadow-2xl overflow-hidden ${mobileTab !== 'chat' ? 'hidden lg:flex' : 'flex'}`}>
                ...
                <form onSubmit={sendChat} className="p-4 flex gap-2">
                  <input type="text" value={chatInput} onChange={e => setChatInput(e.target.value)} placeholder="Type..." className="flex-grow bg-slate-800 border border-slate-700 rounded-full px-5 py-2 text-xs outline-none focus:ring-1 focus:ring-amber-500" />
